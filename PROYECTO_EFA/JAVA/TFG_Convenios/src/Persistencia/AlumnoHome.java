 ...
 * @see Persistencia.Alumno
 * @author Hibernate Tools
 */
public class AlumnoHome {

	private static final Logger logger = Logger.getLogger(AlumnoHome.class.getName());
	...
	public Alumno findById(java.lang.String id) {
		...
		try {
			Alumno instance = (Alumno) sessionFactory.getCurrentSession().get("persistencia.Alumno", id);
			if (instance == null) {
	...
	public List findByExample(Alumno instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("persistencia.Alumno")
